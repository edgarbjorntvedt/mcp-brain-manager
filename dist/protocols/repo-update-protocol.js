...
`;
    ...
    createBrainStateStep(options) {
        const instructions = [];
        const timestamp = new Date().toISOString();
        // Get current projects index
        instructions.push(BrainToolInstructions.stateGet('system', 'projects_index'));
        ...
        instructions.push(BrainToolInstructions.stateSet('system', 'last_session_context', {
            timestamp,
    ...
    createSummaryStep(options, summary) {
        const instructions = [];
...
Generated by Repository Update Protocol`;
    ...
    generateCommitMessage(options) {
        ...
        lines.push('\n- Update documentation', '- Update brain state', '- General maintenance');
        return lines.join('\n');
    }
    /**
     * Generate a quick summary without executing steps
     */
    generateSummaryOnly(projectName, changes, notes) {
        const date = new Date().toISOString().split('T')[0];
...
//# sourceMappingURL=repo-update-protocol.js.map