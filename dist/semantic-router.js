...
export class SemanticRouter {
    ...
    async classify(message, context) {
        ...
        classification.reasoning = this.generateReasoning(message, classification.mode, classification.confidence, context);
        return classification;
    }
    performClassification(messageLower, context) {
        ...
        if (context?.lastProject) {
            ...
            const hasContinuationSignal = continuationSignals.some(signal => messageLower.includes(signal));
    ...
    scoreKeywords(message, keywords) {
        let score = 0;
    ...
    calculateConfidence(scores, bestMode) {
        ...
        const secondBest = Math.max(...values.filter(v => v !== bestScore));
    ...
    generateReasoning(message, mode, confidence, context) {
        const reasons = [];
    ...
    getDetectedKeywords(message, mode) {
        const messageLower = message.toLowerCase();
        const detected = [];
        const keywordMap = {
    ...
    async improveClassification(message, context, feedback) {
